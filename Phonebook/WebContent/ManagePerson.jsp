...
<body bgcolor="#ADD8E6">
	...
	<%
		...
		show_phones = jsp_parameters.get("show_phones");
	%>
				...
				<td>
					<table>
						...
						<tr>
							...
							<td><a
								href="<%=request.getContextPath()%>/?action=edit_phone&id=<%=person.getId()%>&phone=<%=phone%>">Редактировать</a>
							</td>
							<td><a
								href="<%=request.getContextPath()%>/?action=delete_phone&id=<%=person.getId()%>&phone=<%=phone%>">Удалить</a>
							</td>
						</tr>
						<%
							}
					...
					href="<%=request.getContextPath()%>/?action=add_phone&id=<%=person.getId()%>">Добавить</a>
				</td>
			</tr>
			<%
				}
			...
			<tr>
				<td colspan="2" align="center"><input type="submit"
					...
					value="<%=jsp_parameters.get("next_action_label")%>" /> <br>
						<a href="<%=request.getContextPath()%>/phonebook_jsp">Вернуться
							к списку</a></td>
